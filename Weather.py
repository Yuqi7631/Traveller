...
class Weather(Frame):
    ...
    def create_widgets(self):
        ...
        self.canvas.create_text(400, 100, text="Today's weather", font=("Arial", 20, "bold"), fill="white")
        
        # Labels and Entry
        self.canvas.create_text(280, 170, text="City Name", font=("Arial", 15, "bold"), fill="black")
        self.city_entry = Entry(self, width=30)
        self.canvas.create_window(520, 170, window=self.city_entry, width=200, height=40)

        self.canvas.create_text(280, 220, text="Country Name", font=("Arial", 15, "bold"), fill="black")
        self.country_entry = Entry(self, width=30)
        self.canvas.create_window(520, 220, window=self.country_entry, width=200, height=40)

        self.search_button = Button(self, text="Query Weather",
                                    command=lambda: self.fetch_weather(
                                        self.city_entry.get().strip(), self.country_entry.get().strip()
                                    ))
        self.canvas.create_window(400, 290, window=self.search_button, width=200, height=40)

        self.location_button = Button(self, text="Get Weather for Current Location", command=lambda: self.fetch_weather())
        self.canvas.create_window(400, 350, window=self.location_button, width=200, height=40)

        self.weather_label = Label(self, textvariable=self.weather_result, justify="left", anchor="w", bg="white")
        self.canvas.create_window(400, 480, window=self.weather_label, width=500, height=180)
    ...
    def fetch_weather(self, city=None, country=None):
        ...
        if city is None:  
            ...
            if city is None:
                ...
                return
            messagebox.showinfo("Location Detected", f"Detected location: {city}, {country}")
        try:
