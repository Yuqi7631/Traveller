...
class LoginPage(Frame):
    ...
    def create_widgets(self):
        ...
        self.canvas.create_text(
            200, 50, text="Traveller", font=("Arial", 20, "bold"), fill="white"
        )
        ...
        self.canvas.create_text(
            80, 100, text="Username", font=("Arial", 10, "bold"), fill="black"
        )
        self.canvas.create_text(
            80, 140, text="Password", font=("Arial", 10, "bold"), fill="black"
        )
        ...
        self.canvas.create_window(
            200, 180, window=self.button_login, width=100, height=25
        )
        ...
        self.canvas.create_window(
            200, 220, window=self.button_signup, width=100, height=25
        )
        ...
        self.canvas.create_window(
            200, 260, window=self.button_quit, width=100, height=25
        )
    ...
    def login_check(self):
        """If username exists and password is correct, show MainPage"""
            ...
            for row in reader:
                if row[0] == name:
                    ...
                    hashed_pwd = hashlib.pbkdf2_hmac(
                        "SHA-512", pwd.encode(), hex_salt_store, 100000
                    )
    ...
    def signup(self):
        """Signup before login, store username and password"""
            ...
            for row in reader:
                if row[0] == name:
                    messagebox.showerror(
                        title="Failed", message="Username already exists!"
                    )
                    return
        with open(self.signup_file, mode="a", newline="") as f:
            ...
            messagebox.showinfo(
                title="Congratulations!", message="Signup Successfully!"
            )
